    ...
    if (delegate == null) {
      if (SchemaClass.INDEX_TYPE.SPATIAL.name().equalsIgnoreCase(im.getType())) {
        if (im.getIndexDefinition().getFields().size() > 1) {
          delegate =
              new LuceneLegacySpatialIndexEngine(storage, indexName, id, OShapeFactory.INSTANCE);
        } else {
          delegate =
              new LuceneGeoSpatialIndexEngine(storage, indexName, id, OShapeFactory.INSTANCE);
        }
