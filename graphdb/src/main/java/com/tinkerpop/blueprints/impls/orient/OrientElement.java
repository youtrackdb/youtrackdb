...
/**
 * Base Graph Element where OrientVertex and OrientEdge classes extends from. Labels are managed as
 * OrientDB classes.
 *
...
public abstract class OrientElement
  ...
  protected OrientElement(final OrientBaseGraph rawGraph, final OIdentifiable iRawElement) {
    if (classicDetachMode) graph = rawGraph;
    else graph = null;
  ...
  /**
   * (Blueprints Extension) Sets multiple properties in one shot against Vertices and Edges. This
   * improves performance avoiding to save the graph element at every property set.<br>
   * Example:
   *
   * <p><code>
  ...
  public <T extends OrientElement> T setProperties(final Object... fields) {
    if (checkDeletedInTx())
      graph.throwRecordNotFoundException(
          getIdentity(), "The graph element " + getIdentity() + " has been deleted");
  ...
  public <T> T getProperty(final String key) {
    ...
    if (graph != null
      ...
      return (T) fieldValue;
        ...
        && OMultiValue.isMultiValue(fieldValue)
      ...
      if (firstValue instanceof ODocument) {
        ...
        /// clusterId -2 Is considered a projection so does not have a class but is a not embedded
        // record
        if (document.getIdentity().getClusterId() != -2
            && (document.isEmbedded()
  ...
  /**
   * (Blueprints Extension) Saves current element. You don't need to call save() unless you're
   * working against Temporary Vertices.
   */
  public void save() {
    save(null);
  }
  ...
  public ODocument getRecord() {
    ...
    if (rawElement instanceof ODocument) return (ODocument) rawElement;
   ...
   * offline. To reattach it use @attach.
   *
   * <p>This methods works only in "classic detach/attach mode" when dettachment/attachment is done
   * manually, by default it is done automatically, and currently active graph connection will be
   * used as graph elements owner.
   *
   ...
   * Use this method to pass elements between graphs or to switch between Tx and NoTx instances.
   *
   * <p>This methods works only in "classic detach/attach mode" when detachment/attachment is done
   * manually, by default it is done automatically, and currently active graph connection will be
   * used as graph elements owner.
   *
   * <p>To set "classic detach/attach mode" please set custom database parameter <code>
   * classicDetachMode</code> to <code>true</code>.
   *
  ...
  public OrientElement attach(final OrientBaseGraph iNewGraph) {
    if (iNewGraph == null) throw new IllegalArgumentException("Graph is null");
   ...
   * @param value property value
   */
  public final void validateProperty(final Element element, final String key, final Object value)
  ...
  public void reload() {
    ...
    if (rec != null) rec.reload(null, true);
  }
  ...
  protected String checkForClassInSchema(final String className) {
    ...
    if (!schema.existsClass(className)) {
      ...
      try {
        OCallable<OClass, OrientBaseGraph> outsideTx =
  ...
  protected void setPropertyInternal(
      final Element element, final ODocument doc, final String key, final Object value) {
