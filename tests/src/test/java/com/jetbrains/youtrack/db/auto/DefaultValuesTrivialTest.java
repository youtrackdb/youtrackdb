...
public class DefaultValuesTrivialTest {
  ...
  public void test() {
    ...
    var dtStart = getDatabaseSysdate(db);
    ...
    var dtAfter = getDatabaseSysdate(db);
    for (var i = 0; i < DOCUMENT_COUNT; ++i) {
  ...
  public void testPrepopulation() {
    ...
    {
      var doc = ((EntityImpl) db.newEntity(classA.getName()));
  ...
  public void testPrepopulationIndex() {
...

    {
      ...
      db.save(doc);
      db.commit();
      try (var stream = db.getIndex("ClassA.name").getInternal()
  ...
  public void testPrepopulationIndexTx() {
...

    {
      ...
      try (var stream = index.getInternal().getRids(db, "default name")) {
        assertEquals(1, stream.count());
      }
    }
  }
  ...
  public void testPrepopulationMultivalueIndex() {
...

    {
      ...
      try (var stream = index.getInternal().getRids(db, new CompositeKey("1"))) {
        assertEquals(1, stream.count());
      }
    }
    {
      ...
      try (var stream = index.getInternal().getRids(db, new CompositeKey("2"))) {
        assertEquals(1, stream.count());
      }
    }
    try (var stream = index.getInternal().getRids(db, new CompositeKey("3"))) {
