...
class JSONTest @Parameters(value = ["remote"]) constructor(@Optional remote: Boolean?) :
    ...
    fun testEmbeddedList() {
        ...
        list.add((db.newEntity() as EntityImpl).field("name", "Luca"))
        list.add((db.newEntity() as EntityImpl).field("name", "Marcus"))
        ...
        Assert.assertTrue(loadedDoc.field<Any>("embeddedList") is List<*>)
        Assert.assertTrue(
            (loadedDoc.field<List<EntityImpl?>>("embeddedList")).first() is EntityImpl
        )

        var newDoc = (loadedDoc.field<List<EntityImpl>>("embeddedList"))[0]
        Assert.assertEquals(newDoc.field("name"), "Luca")
        newDoc = (loadedDoc.field<List<EntityImpl>>("embeddedList"))[1]
    ...
    companion object {
        const val FORMAT_WITHOUT_TYPES: String =
