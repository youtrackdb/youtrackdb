    ...
    if (input instanceof OIdentifiable) {
      ...
      if (rec instanceof ODocument) {
        ...
        if (setOperation) {
          ...
          log(Level.DEBUG, "set %s=%s in document=%s", fieldName, newValue, doc);
        } else {
          if (fieldName != null) {
            final Object prev = doc.removeField(fieldName);
            log(Level.DEBUG, "removed %s (value=%s) from document=%s", fieldName, prev, doc);
          } else {
            for (String f : fieldNames) {
              final Object prev = doc.removeField(f);
              log(Level.DEBUG, "removed %s (value=%s) from document=%s", f, prev, doc);
            }
          }
        }

        if (save) {
          log(Level.DEBUG, "saving record %s", doc.toJSON());
          db.save(doc);
          log(Level.DEBUG, "saved record %s", doc.toJSON());
        }
      }
    }
