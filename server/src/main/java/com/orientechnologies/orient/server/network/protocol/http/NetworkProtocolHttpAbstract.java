...
public abstract class NetworkProtocolHttpAbstract extends NetworkProtocol
  ...
  public NetworkProtocolHttpAbstract(OServer server) {
    ...
    while (!channel.socket.isInputShutdown()) {
      ...
      if (currChar == '\r') {
        if (request.length() > 0 && !endOfHeaders) {
          ...
          if (StringSerializerHelper.startsWithIgnoreCase(line, OHttpUtils.HEADER_AUTHORIZATION)) {
          ...
          } else if (StringSerializerHelper.startsWithIgnoreCase(line, OHttpUtils.HEADER_COOKIE)) {
            ...
            if (StringSerializerHelper.startsWithIgnoreCase(
              ...
              line, OHttpUtils.HEADER_IF_MATCH)) {
