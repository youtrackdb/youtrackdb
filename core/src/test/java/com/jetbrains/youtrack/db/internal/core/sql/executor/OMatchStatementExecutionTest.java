...
public class OMatchStatementExecutionTest extends DBTestBase {
  ...
  public void testSimple() throws Exception {
    List<EntityImpl> qResult = collect(
  ...
  public void testTriangle2Old() {
    ...
    EntityImpl friend1 = ((YTIdentifiable) doc.field("friend1")).getRecord();
    EntityImpl friend2 = ((YTIdentifiable) doc.field("friend2")).getRecord();
    EntityImpl friend3 = ((YTIdentifiable) doc.field("friend3")).getRecord();
  ...
  public void testArrayNumber() {
    ...
    List<?> result = collect(db.command(query));
    assertEquals(1, result.size());
    EntityImpl doc = (EntityImpl) result.get(0);
  ...
  public void testBigEntryPoint() {
    ...
    for (int i = 0; i < 1000; i++) {
      db.begin();
      EntityImpl doc = db.newInstance("testBigEntryPoint1");
      doc.field("a", i);
      doc.save();
      db.commit();
    }
    ...
    db.begin();
    EntityImpl doc = db.newInstance("testBigEntryPoint2");
    doc.field("b", "b");
    doc.save();
    db.commit();
  ...
  private List<EntityImpl> collect(YTResultSet set) {
    return set.stream()
        .map(
            (r) -> {
              if (r.isEntity()) {
                return (EntityImpl) r.getEntity().get();
              } else {
                return (EntityImpl) r.toEntity();
              }
            })
  ...
  private List<YTIdentifiable> collectIdentifiable(YTResultSet set) {
    return set.stream().map((r) -> (EntityImpl) r.toEntity()).collect(Collectors.toList());
  }
