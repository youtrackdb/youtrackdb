package com.orientechnologies.core.storage.index.sbtree.singlevalue.v3;
...
public class CellBTreeSingleValueV3TestIT {
  ...
  public void testKeyDelete() throws Exception {
    ...
    for (int i = 0; i < keysCount; i++) {
      ...
      atomicOperationsManager.executeInsideAtomicOperation(
          ...
          atomicOperation ->
              singleValueTree.put(
                  atomicOperation, Integer.toString(k), new YTRecordId(k % 32000, k)));
    }
    final int rollbackInterval = 100;
            ...
            atomicOperation -> {
              ...
              if (key > 0 && key % 100_000 == 0) {
                for (int keyToVerify = 0; keyToVerify < keysCount; keyToVerify++) {
                  if (keyToVerify > key) {
                    Assert.assertEquals(
                        new YTRecordId(keyToVerify % 32000, keyToVerify),
  ...
  public void testKeyAddDeleteHalf() throws Exception {
    ...
    for (int iterations = 0; iterations < 4; iterations++) {
      ...
      for (int i = 0; i < keysCount / 2; i++) {
        ...
        Assert.assertEquals(
            singleValueTree.get(Integer.toString(key)), new YTRecordId(key % 32000, key));
      }
      ...
      for (int i = 0; i < (iterations + 2) * keysCount / 2; i++) {
        if (i < start) {
          ...
          Assert.assertEquals(
              new YTRecordId(i % 32000, i), singleValueTree.get(Integer.toString(i)));
        }
      }
  ...
  public void testIterateEntriesBetweenString() throws Exception {
    ...
    final NavigableMap<String, YTRID> keyValues = new TreeMap<>();
    final Random random = new Random();
    try {
      atomicOperationsManager.executeInsideAtomicOperation(
          null,
          atomicOperation -> {
            for (int j = 0; j < keysCount; j++) {
              ...
              singleValueTree.put(atomicOperation, key, new YTRecordId(clusterId, val));
              System.out.println("Added key=" + key + ", value=" + val);

              keyValues.put(key, new YTRecordId(clusterId, val));
            }
          });
  ...
  private void assertIterateBetweenEntriesNonRandom(
      ...
      final NavigableMap<String, YTRID> keyValues,
    ...
    for (int i = startFrom; i < keyValues.size(); i++) {
      ...
      final Iterator<ORawPair<String, YTRID>> indexIterator;
      try (final Stream<ORawPair<String, YTRID>> stream =
          singleValueTree.iterateEntriesBetween(
              fromKey, fromInclusive, toKey, toInclusive, ascSortOrder)) {
        indexIterator = stream.iterator();
