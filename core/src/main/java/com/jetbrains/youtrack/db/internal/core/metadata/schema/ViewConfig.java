...
public class ViewConfig {
  ...
  protected List<ViewIndexConfig> indexes = new ArrayList<>();
  ...
  public ViewConfig(String name, String query) {
    this.name = name;
    this.query = query;
  }

  public ViewConfig copy() {
    ViewConfig result = new ViewConfig(this.name, this.query);
    result.updatable = this.updatable;
    for (ViewIndexConfig index : indexes) {
      ViewIndexConfig idx = result.addIndex(index.type, index.engine);
  ...
  public ViewIndexConfig addIndex(String type, String engine) {
    ViewIndexConfig result = new ViewIndexConfig(type, engine);
    indexes.add(result);
    return result;
  }
  ...
  public List<ViewIndexConfig> getIndexes() {
    return indexes;
  }
