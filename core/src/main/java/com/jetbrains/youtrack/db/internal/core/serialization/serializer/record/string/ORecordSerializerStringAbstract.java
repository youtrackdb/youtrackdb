...
import com.jetbrains.youtrack.db.internal.core.serialization.serializer.string.OStringSerializerAnyStreamable;
...
public abstract class ORecordSerializerStringAbstract implements ORecordSerializer, Serializable {
  ...
  public static Object fieldTypeFromStream(
      YTDatabaseSessionInternal db, final EntityImpl iDocument, YTType iType,
      final Object iValue) {
    if (iValue == null) {
      return null;
    }
    ...
    switch (iType) {
      ...
      case EMBEDDED: {
        ...
        if (embeddedObject instanceof EntityImpl) {
          ODocumentInternal.addOwner((EntityImpl) embeddedObject, iDocument);
        }
      ...
      case CUSTOM:
        ...
        if (result instanceof EntityImpl) {
          ODocumentInternal.addOwner((EntityImpl) result, iDocument);
        }
        return result;
    ...
    switch (iType) {
      ...
      case EMBEDDED:
        if (iValue instanceof EntityImpl) {
          ORecordSerializerSchemaAware2CSV.INSTANCE.toString((EntityImpl) iValue, iBuffer, null);
        } else {
  ...
  public abstract RecordAbstract fromString(
      YTDatabaseSessionInternal db, String iContent, RecordAbstract iRecord, String[] iFields);
  ...
  public Record fromString(YTDatabaseSessionInternal db, final String iSource) {
  ...
  public byte[] toStream(YTDatabaseSessionInternal session, final RecordAbstract iRecord) {
    final long timer = PROFILER.startChrono();
