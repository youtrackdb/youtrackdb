...
public class StorageConfigurationImpl implements SerializableStream, StorageConfiguration {

  private static final RecordId CONFIG_RID = new ImmutableRecordId(0, 0);

  protected final ReentrantReadWriteLock lock = new ReentrantReadWriteLock();

  private String charset;
  private final List<StorageEntryConfiguration> properties = new ArrayList<>();
  ...
  private StorageSegmentConfiguration fileTemplate;
  ...
  protected StorageConfigurationUpdateListener updateListener;
  ...
  public StorageConfigurationImpl(
    ...
    try {
      ...
      initConfiguration(new ContextConfiguration());
      clear();
    } finally {
  ...
  public void initConfiguration(ContextConfiguration conf) {
    lock.writeLock().lock();
    try {
  ...
  private void clear() {
    fileTemplate = new StorageSegmentConfiguration();
    ...
    validation = getContextConfiguration().getValueAsBoolean(GlobalConfiguration.DB_VALIDATION);
  ...
  public void fromStream(final byte[] stream, int offset, int length, Charset charset) {
    ...
    try {
      ...
      if (version < 14) {
        throw new StorageException("cannot open database created with a version before 2.0 ");
      }
      ...
      for (var i = 0; i < size; ++i) {
        ...
        switch (clusterType) {
          case "d":
            ...
            currentCluster =
                new StoragePaginatedClusterConfiguration(
                    clusterId,
                    clusterName,
                    null,
                    cc,
  ...
  public byte[] toStream(Charset charset) throws SerializationException {
  ...
  public void create() throws IOException {
    lock.writeLock().lock();
    try {
      storage.createRecord(CONFIG_RID, new byte[]{0, 0, 0, 0}, 0, Blob.RECORD_TYPE, null);
    } finally {
      lock.writeLock().unlock();
    }
  }
  ...
  public void addCluster(StoragePaginatedClusterConfiguration config) {
    lock.writeLock().lock();
    try {
  ...
  public void setProperty(final String iName, final String iValue) {
    ...
    try {
      ...
      // NOT FOUND: CREATE IT
      properties.add(new StorageEntryConfiguration(iName, iValue));
    } finally {
      lock.writeLock().unlock();
    }
  }
  ...
  private int phySegmentFromStream(
      final String[] values, int index, final StorageSegmentConfiguration iSegment) {
  ...
  private void phySegmentToStream(
      final StringBuilder iBuffer, final StorageSegmentConfiguration iSegment) {
  ...
  private void fileToStream(final StringBuilder iBuffer, final StorageFileConfiguration iFile) {
    write(iBuffer, iFile.path);
    write(iBuffer, iFile.type);
    write(iBuffer, iFile.maxSize);
  }

  private void entryToStream(final StringBuilder iBuffer, final StorageEntryConfiguration iEntry) {
  ...
  public void setConfigurationUpdateListener(StorageConfigurationUpdateListener updateListener) {
    lock.writeLock().lock();
    try {
