...
public class OSchedulerImpl {
  ...
  public void load(YTDatabaseSessionInternal database) {
    if (database.getMetadata().getSchema().existsClass(OScheduledEvent.CLASS_NAME)) {
      final Iterable<EntityImpl> result = database.browseClass(OScheduledEvent.CLASS_NAME);
  ...
  public void initScheduleRecord(YTDatabaseSessionInternal session, EntityImpl doc) {
  ...
  public void preHandleUpdateScheduleInTx(YTDatabaseSessionInternal session, EntityImpl doc) {
    try {
