...
public class ORecordSerializerSchemaAware2CSV extends ORecordSerializerCSVAbstract {
  ...
  public RecordAbstract fromString(
      YTDatabaseSessionInternal db, String iContent, final RecordAbstract iRecord,
    ...
    if (pos > -1 && (pos < posFirstValue || posFirstValue == -1)) {
      if ((record.getIdentity().getClusterId() < 0 || database == null)) {
        ODocumentInternal.fillClassNameIfNeeded(((EntityImpl) iRecord),
      ...
      try {
        ...
        if (pos > -1) {
          ...
          if ("@class".equals(fieldName)) {
            ODocumentInternal.fillClassNameIfNeeded(((EntityImpl) iRecord), value.toString());
          } else {
  ...
  @Override
  public byte[] toStream(YTDatabaseSessionInternal session, RecordAbstract iRecord) {
