 ...
 * @since 10/21/14
 */
public class ImmutableSchema implements Schema {
  ...
  private final Int2ObjectOpenHashMap<SchemaView> clustersToViews;
  private final Map<String, SchemaView> views;
  ...
  public ImmutableSchema(SchemaShared schemaShared, DatabaseSessionInternal database) {
    ...
    views = new HashMap<String, SchemaView>(schemaShared.getViews(database).size());

    for (SchemaView oClass : schemaShared.getViews(database)) {
      final SchemaImmutableView immutableClass = new SchemaImmutableView(database, oClass, this);
    ...
    for (SchemaClass cl : views.values()) {
      ((SchemaImmutableClass) cl).init();
    }
  }
  ...
  @Override
  public SchemaView getView(String name) {
    if (name == null) {
      return null;
    }

    SchemaView cls = views.get(name.toLowerCase(Locale.ENGLISH));
    return cls;
  }
