...
public final class OPaginatedClusterV1 extends OPaginatedCluster {
  ...
  private static final class FindFreePageResult {
    ...
    private FindFreePageResult(
        final long pageIndex, final int freePageIndex, final boolean allocateNewPage) {
      this.pageIndex = pageIndex;
  ...
  public OPhysicalPosition allocatePosition(
      final byte recordType, final OAtomicOperation operation) {
  ...
  public OPhysicalPosition createRecord(
    ...
    final byte[] encryptedContent = encryption.encrypt(content);
  ...
  @Override
  public boolean deleteRecord(OAtomicOperation operation, final long clusterPosition) {
    return calculateInsideComponentOperation(
        operation,
        atomicOperation -> {
          acquireExclusiveLock();
          try {
            final OClusterPositionMapBucket.PositionEntry positionEntry =
                clusterPositionMap.get(clusterPosition, operation);
            if (positionEntry == null) {
              return false;
            }
            ...
            do {
              ...
              OCacheEntry cacheEntry = loadPageForWrite(operation, fileId, pageIndex, true);
              ...
              try {
                ...
                if (localPage.isDeleted(recordPosition)) {
                  ...
                  cacheEntry = loadPageForWrite(operation, fileId, pageIndex, true);

                  localPage = new OClusterPage(cacheEntry);
                }

                final byte[] content = localPage.deleteRecord(recordPosition, true);
                operation.addDeletedRecordPosition(id, cacheEntry.getPageIndex(), recordPosition);
                assert content != null;
              ...
              updateFreePagesIndex(initialFreePageIndex, pageIndex, operation);
            ...
            do {
              ...
              if (isNew) {
                try (final OCacheEntry stateCacheEntry =
                  ...
                  final OPaginatedClusterStateV1 clusterState =
                      new OPaginatedClusterStateV1(stateCacheEntry);
                ...
                cacheEntry = loadPageForWrite(atomicOperation, fileId, nextPageIndex, true);
              }
  ...
  private void updateClusterState(
    ...
    try (final OCacheEntry pinnedStateEntry =
        loadPageForWrite(atomicOperation, fileId, STATE_ENTRY_INDEX, true)) {
      final OPaginatedClusterStateV1 paginatedClusterState =
          new OPaginatedClusterStateV1(pinnedStateEntry);
  ...
  private void updateFreePagesIndex(
    ...
    try (final OCacheEntry cacheEntry =
      ...
      if (newFreePageIndex >= 0) {
        ...
        try (final OCacheEntry pinnedStateEntry =
            loadPageForRead(atomicOperation, fileId, STATE_ENTRY_INDEX)) {
          final OPaginatedClusterStateV1 clusterFreeList =
              new OPaginatedClusterStateV1(pinnedStateEntry);
  ...
  private void initCusterState(final OAtomicOperation atomicOperation) throws IOException {
    ...
    try {
      final OPaginatedClusterStateV1 paginatedClusterState =
          new OPaginatedClusterStateV1(stateEntry);
  ...
  public OClusterBrowsePage nextPage(final long lastPosition) throws IOException {
    ...
    try {
      ...
      try {
        ...
        final OClusterPositionMapV1.OClusterPositionEntry[] nextPositions =
