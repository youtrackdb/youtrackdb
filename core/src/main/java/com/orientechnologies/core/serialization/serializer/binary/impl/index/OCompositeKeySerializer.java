...
package com.orientechnologies.core.serialization.serializer.binary.impl.index;
...
public class OCompositeKeySerializer implements OBinarySerializer<OCompositeKey> {
  ...
  public int getObjectSize(OCompositeKey compositeKey, Object... hints) {
    final YTType[] types = getKeyTypes(hints);
  ...
  private YTType[] getKeyTypes(Object[] hints) {
    final YTType[] types;
    ...
    for (int i = 0; i < keys.size(); i++) {
      ...
      if (key != null) {
        ...
        if (key instanceof Map
            && !(type == YTType.EMBEDDEDMAP || type == YTType.LINKMAP)
            && ((Map<?, ?>) key).size() == 1
            && ((Map<?, ?>) key)
            .keySet()
            .iterator()
