...
import com.orientechnologies.core.serialization.OBinaryProtocol;
...
public class YTRecordId implements YTRID {
  ...
  protected YTRecordId() {
  }

  public YTRecordId(final int clusterId, final long position) {
    this.clusterId = clusterId;
    checkClusterLimits();
    clusterPosition = position;
  }

  public YTRecordId(final int iClusterIdId) {
    clusterId = iClusterIdId;
    checkClusterLimits();
  }

  public YTRecordId(final String iRecordId) {
   ...
   */
  public YTRecordId(final YTRID parentRid) {
  ...
  public int compare(final YTIdentifiable obj1, final YTIdentifiable obj2) {
    if (obj1 == obj2) {
      return 0;
    }
  ...
  public YTRecordId fromStream(final InputStream iStream) throws IOException {
    clusterId = OBinaryProtocol.bytes2short(iStream);
    clusterPosition = OBinaryProtocol.bytes2long(iStream);
    return this;
  }

  public YTRecordId fromStream(final OMemoryStream iStream) {
    clusterId = iStream.getAsShort();
    clusterPosition = iStream.getAsLong();
    return this;
  }

  public YTRecordId fromStream(final byte[] iBuffer) {
    if (iBuffer != null) {
  ...
  @Override
  public YTRID nextRid() {
    return new YTRecordId(clusterId, clusterPosition + 1);
  }
  ...
  public static void serialize(YTRID id, DataOutput output) throws IOException {
    if (id == null) {
      output.writeInt(-2);
      output.writeLong(-2);
    } else {
  ...
  public static YTRecordId deserialize(DataInput input) throws IOException {
    int cluster = input.readInt();
    long pos = input.readLong();
    if (cluster == -2 && pos == -2) {
      return null;
    }
    return new YTRecordId(cluster, pos);
  }
}
